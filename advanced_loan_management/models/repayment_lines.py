...
class RepaymentLine(models.Model):
    ...
    def action_pay_emi(self):
        ...
        interest_product_id = self.company_id.interest_product_id.id
        # repayment_product_id = self.env['ir.config_parameter'].sudo().get_param(
        #     'advanced_loan_management.repayment_product_id')
        repayment_product_id = self.company_id.repayment_product_id.id
        for rec in self:
...
class RepaymentsLinesPartner(models.Model):
    """Partner loan repayments """
    _name = "repayment.line.partner"
    _description = "Partner loan repayments"
    ...
    total_amount = fields.Float(string="Total Amount", required=True,
                                help="Total Amount", digits=(16, 6))
    outstanding_capital = fields.Float(string="Outstanding capital", required=True,
                                help="Outstanding capital", digits=(16, 6))
    loan_id = fields.Many2one('loan.request', string="Loan Ref.",
                              help="Loan",
                              readonly=True)